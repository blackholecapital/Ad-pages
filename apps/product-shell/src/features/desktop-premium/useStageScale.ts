...
export type StageFitMode = "contain" | "cover";
...
export function useStageScale(
  ...
  const { fullPublishedViewport = false, fitMode = "contain" } = options;
  ...
  useEffect(() => {
    ...
    function update() {
      ...
      const s = fitMode === "cover"
        ? Math.max(vw / stage.w, vh / stage.h)
        : Math.min(vw / stage.w, vh / stage.h);
  ...
  }, [containerRef, stage.w, stage.h, fullPublishedViewport, fitMode]);
